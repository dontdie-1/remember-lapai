...
import {
  ...
  Mic, GraduationCap, AlertTriangle, Briefcase, PhoneCall, MapPin, Plus as PlusIcon,
} from "lucide-react";
...
const missingPersons = [
  { name: "Musa Aliyu", age: 74, ward: "Gulu Ward", lastSeen: "Aug 18, 2026", desc: "Elderly, walks with a stick, last seen near the central market wearing a blue kaftan.", status: "Missing" },
  { name: "Zainab Ibrahim", age: 9, ward: "Lapai Central", lastSeen: "Aug 20, 2026", desc: "Found and reunited with family on Aug 21, 2026.", status: "Found" },
];

const jobListings = [
  { title: "Shop Assistant", org: "Lapai Central Market Stores", ward: "Lapai Central", type: "Full-time", posted: "2 days ago" },
  { title: "Tailoring Apprentice", org: "Amina Fashion House", ward: "Gulu Ward", type: "Apprenticeship", posted: "5 days ago" },
  { title: "IT Support Intern", org: "IBB University ICT Unit", ward: "Lapai Central", type: "Internship", posted: "1 week ago" },
];

const emergencyContacts = [
  { name: "Lapai General Hospital", type: "Hospital", phone: "064-880-112", ward: "Lapai Central" },
  { name: "Lapai Divisional Police HQ", type: "Police", phone: "064-880-201", ward: "Lapai Central" },
  { name: "Niger State Fire Service, Lapai Unit", type: "Fire Service", phone: "064-880-333", ward: "Lapai Central" },
  { name: "Gulu Ward Primary Health Centre", type: "Clinic", phone: "080-3456-7890", ward: "Gulu Ward" },
];
...
function HomePage({ go }) {
  ...
  const actions = [
    ...
    { icon: AlertTriangle, label: "Missing Persons", tab: "missing" },
    { icon: Briefcase, label: "Jobs & Apprenticeships", tab: "jobs" },
    { icon: PhoneCall, label: "Emergency Contacts", tab: "emergency" },
  ];
...
function MissingPersonsBoard({ goHome }) {
  const [showForm, setShowForm] = useState(false);
  return (
    <div style={{ paddingBottom: 90 }}>
      <TopHeader title="Missing Persons" onBack={goHome} />
      <div style={{ padding: 20 }}>
        <div style={{ fontSize: 12, color: C.textDim, marginBottom: 16, lineHeight: 1.6 }}>
          Visible to anyone, account or no account, so the whole community can help look.
        </div>
        <button
          onClick={() => setShowForm(true)}
          style={{ width: "100%", background: C.navy, color: "#fff", border: "none", borderRadius: 10, padding: "14px", fontSize: 14, fontWeight: 700, cursor: "pointer", marginBottom: 20 }}
        >
          + Report a Missing Person
        </button>
        {missingPersons.map((p) => (
          <div key={p.name} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 12, display: "flex", gap: 12 }}>
            <Avatar size={54} seed={p.name.length} />
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontWeight: 700, fontSize: 15, color: C.text }}>{p.name}, {p.age}</div>
                <span style={{
                  fontSize: 10, fontWeight: 700, padding: "3px 9px", borderRadius: 999,
                  background: p.status === "Missing" ? "#F5D9D2" : "#DCEEE9",
                  color: p.status === "Missing" ? C.rust : C.green,
                }}>{p.status.toUpperCase()}</span>
              </div>
              <div style={{ fontSize: 12, color: C.textDim, margin: "4px 0" }}>{p.ward} · Last seen {p.lastSeen}</div>
      ...
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }} onClick={() => setShowForm(false)}>
          <div style={{ background: "#fff", borderRadius: 14, padding: 24, width: 340, maxWidth: "88vw" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 12 }}>Report a Missing Person</div>
            <div style={{ fontSize: 13, color: C.textDim, marginBottom: 16 }}>Full name, age, last seen location and description would go here. Demo only.</div>
            <button onClick={() => setShowForm(false)} style={{ width: "100%", background: C.navy, color: "#fff", border: "none", borderRadius: 8, padding: 12, fontWeight: 700 }}>Close</button>
          </div>
        </div>
      )}
    </div>
...
function JobBoard({ goHome }) {
  const [showForm, setShowForm] = useState(false);
  return (
    <div style={{ paddingBottom: 90 }}>
      <TopHeader title="Jobs & Apprenticeships" onBack={goHome} />
      <div style={{ padding: 20 }}>
        <div style={{ fontSize: 12, color: C.textDim, marginBottom: 16, lineHeight: 1.6 }}>
          Any local business or individual can post an opening directly.
        </div>
        <button
          onClick={() => setShowForm(true)}
          style={{ width: "100%", background: C.navy, color: "#fff", border: "none", borderRadius: 10, padding: "14px", fontSize: 14, fontWeight: 700, cursor: "pointer", marginBottom: 20 }}
        ...
        {jobListings.map((j) => (
          <div key={j.title} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <Briefcase size={16} color={C.gold} />
              <div style={{ fontWeight: 700, fontSize: 15, color: C.text }}>{j.title}</div>
            </div>
            <div style={{ fontSize: 13, color: C.textDim }}>{j.org}</div>
            <div style={{ display: "flex", gap: 10, fontSize: 12, color: C.textDim, marginTop: 6 }}>
              <span>{j.ward}</span>·<span>{j.type}</span>·<span>{j.posted}</span>
            </div>
          </div>
        ))}
      </div>
      {showForm && (
        <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }} onClick={() => setShowForm(false)}>
          <div style={{ background: "#fff", borderRadius: 14, padding: 24, width: 340, maxWidth: "88vw" }} onClick={(e) => e.stopPropagation()}>
            <div style={{ fontWeight: 700, fontSize: 17, marginBottom: 12 }}>Post a Job</div>
            <div style={{ fontSize: 13, color: C.textDim, marginBottom: 16 }}>Job title, business name, ward and type would go here. Demo only.</div>
            <button onClick={() => setShowForm(false)} style={{ width: "100%", background: C.navy, color: "#fff", border: "none", borderRadius: 8, padding: 12, fontWeight: 700 }}>Close</button>
          </div>
        </div>
      )}
...
function EmergencyContacts({ goHome }) {
  return (
    <div style={{ paddingBottom: 90 }}>
      <TopHeader title="Emergency Contacts" onBack={goHome} />
      <div style={{ padding: 20 }}>
        <div style={{ fontSize: 12, color: C.textDim, marginBottom: 16, lineHeight: 1.6 }}>
          Sample contacts for demonstration. Real numbers should be verified with each institution before launch.
        </div>
        {emergencyContacts.map((c) => (
          <a
            key={c.name}
            href={`tel:${c.phone}`}
            style={{ textDecoration: "none", display: "flex", alignItems: "center", gap: 14, background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16, marginBottom: 12 }}
          >
            <div style={{ width: 42, height: 42, borderRadius: "50%", background: "#F3E7CE", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <PhoneCall size={18} color={C.navy} />
            </div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: C.textDim, display: "flex", alignItems: "center", gap: 4 }}>
                <MapPin size={11} /> {c.ward} · {c.type}
              </div>
            </div>
            <div style={{ fontFamily: FONTS.mono, fontSize: 13, color: C.navy, fontWeight: 700 }}>{c.phone}</div>
          </a>
        ))}
      </div>
    </div>
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      {tab === "missing" && <MissingPersonsBoard goHome={() => setTab("home")} />}
      {tab === "jobs" && <JobBoard goHome={() => setTab("home")} />}
      {tab === "emergency" && <EmergencyContacts goHome={() => setTab("home")} />}

      <BottomNav
        tab={["archive"].includes(tab) ? "museum" : ["missing", "jobs", "emergency"].includes(tab) ? "home" : tab}
        setTab={setTab}
      />
    </div>
  );
